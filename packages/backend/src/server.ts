...
import { buildMarketGraph, SUPPORTED_PROTOCOLS, type MarketGraphRequest } from './market-graph'
...
app.get('/api/graph', asyncHandler(async (request, response) => {
  const graph = await buildMarketGraph(readGraphRequest(request.query))
  response.json(graph)
}))

app.post('/api/graph', asyncHandler(async (request, response) => {
  const graph = await buildMarketGraph(readGraphRequest(request.body))
  response.json(graph)
}))
