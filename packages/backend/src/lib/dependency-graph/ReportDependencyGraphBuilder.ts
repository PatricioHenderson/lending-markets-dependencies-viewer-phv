import { isAaveReport, isMapleReport, isMorphoReport, isSparkReport, type KnownReport } from '../reports/protocols'
...
import type {
  ...
  TokenDependency,
  TokenDependencies,
  TokenDependenciesResponse,
} from './types'

export class ReportDependencyGraphBuilder {
  ...
  async build(report: KnownReport): Promise<DependencyGraph> {
    const input = this.readReport(report)
    const graph = this.newGraph(report.chain)
    const root = this.newNode(DEPENDENCY_NODE_TYPE_MARKET, input.market, report.chain, report.marketId)
  ...
  private readReport(report: KnownReport): ReportGraphInput {
    if (isMapleReport(report)) {
      return {
        market: report.poolSymbol || report.poolName,
        protocol: 'Maple',
        loan: report.asset,
        collaterals: [],
      }
    ...
    if (isMorphoReport(report)) {
      const loan = report.loanAsset
      const collateral = report.modifiers.collateralDependencyRobustness.collateralAsset

      return {
        market: `Morpho ${collateral}/${loan}`,
        protocol: 'Morpho',
        loan,
    ...
    if (isAaveReport(report)) {
      return {
        market: this.prefix(report.loanAsset, 'a'),
        protocol: 'Aave V3',
        loan: report.loanAsset,
        collaterals: report.modifiers.collateralDependencyRobustness.collaterals.map((collateral) => collateral.asset),
      }
    }

    if (isSparkReport(report)) {
      return {
        market: this.prefix(report.loanAsset, 'sp'),
        protocol: 'SparkLend',
        loan: report.loanAsset,
        collaterals: report.modifiers.collateralDependencyRobustness.collaterals.map((collateral) => collateral.asset),
      }
  ...
  private prefix(symbol: string, prefix: string): string {
  ...
  private async requestTokenDependencies(graph: Graph, token: string): Promise<TokenDependencies> {
    return this.llmClient.requestJson<TokenDependenciesResponse>({
